...
- **Tests** (regression guard, run before/after changes): from `backend/`,
  ...
  (or `make test`). 391 tests: unit (security/phone/tools/weather adapter/KB
  ...
  cost/price levers, profile gate), plus proactive-alert coverage (gold
  weather-alert engine numbers, SMS adapter MockTransport paths, season-plan
  persistence, and weather-scan journeys: plan-aware delay, generic fallback,
  dedup/idempotency, outage skip, endpoints + agent tool). LLM +
