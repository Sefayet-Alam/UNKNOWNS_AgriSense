...
| 5 | Financial projection | Itemized cost + yield, revenue, net profit, ROI, break-even; internally consistent (change input → outputs change) | ✅ DONE — pure `Decimal` engine + `calculate_crop_financials`; live CZIS variety yield or explicit farmer override; itemized seeded demo costs/price are visibly labelled and farmer-overridable. Low/base/high, budget fit, two break-even values and math checks are returned. Area/price/yield/cost sensitivity and outage fail-closed paths are tested. Seeded cost/yield/price reference ([backend/app/data/finance_assumptions.json](backend/app/data/finance_assumptions.json)) covers 50 Rabi-season crops — a superset of the 5 crops `generate_season_plan` can build a dated calendar for — so `calculate_crop_financials` answers standalone what-ifs for any of the 50 even when there is no matching season plan; the original 5 cite specific BARC FRG 2024 pages, the rest are disclosed as illustrative estimates without a page citation. |
