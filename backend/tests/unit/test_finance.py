...
def test_projection_changes_correctly_with_area_price_yield_and_cost_inputs():
    ...
    assert higher_price["scenarios"]["base"]["revenue_bdt"] > base["scenarios"]["base"]["revenue_bdt"]
    assert lower_yield["scenarios"]["base"]["revenue_bdt"] < base["scenarios"]["base"]["revenue_bdt"]
...
def test_default_price_and_costs_are_prominently_disclosed_as_demo_values():
    ...
    assert all(
        item["source_type"] == "seeded_demo_value"
...
def test_projection_works_for_a_crop_outside_the_five_focused_crops():
    ...
    assert result["scenarios"]["base"]["revenue_bdt"] - result["total_cost_bdt"] == pytest.approx(
        result["scenarios"]["base"]["net_profit_bdt"], abs=0.01
    )
