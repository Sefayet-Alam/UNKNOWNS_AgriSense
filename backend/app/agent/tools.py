...
def build_season_plan_tool(user):
    ...
    async def generate_season_plan(
        ...
        try:
            ...
            # The farmer still receives the fully grounded plan when persistence
            # is temporarily unavailable; do not claim it was saved.
...
# --------------------------------------------------------------------------- #
# Persisted proactive weather-alert history
# --------------------------------------------------------------------------- #
def build_alerts_tool(user):
    """Return a read-only tool for this farmer's recorded weather alerts."""

    @tool
    async def get_weather_alerts(days_back: int = 7) -> str:
        """Return recent proactive weather alerts sent or recorded for the
        active farm. Use when the farmer asks about an alert, warning, or SMS.
        The records are generated by the forecast scanner; relay them exactly
        and do not invent alerts that are not present.
        """
            ...
            return json.dumps(
                {
                    "status": "NO_ALERTS",
                    "days_back": days_back,
                    "message": "No recorded weather alerts for the active farm.",
                },
                ensure_ascii=False,
            )
        return json.dumps(
